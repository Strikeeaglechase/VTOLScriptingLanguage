...
class Renderer {
	...
	constructor(canvasId?: string | HTMLCanvasElement) {
		this.canvas = typeof canvasId == "object" ? canvasId : (document.getElementById(canvasId ?? "main") as HTMLCanvasElement);
	...
	dashedLine(x: number, y: number, x2: number, y2: number, dash: number[], c?: ColorValue): void {
		this.ctx.strokeStyle = new Color(c).hex;
		this.ctx.beginPath();
		this.ctx.setLineDash(dash);
		this.ctx.moveTo(x, y);
	...
	arc(x: number, y: number, r: number, start: number, end: number, c: ColorValue): void {
		this.ctx.fillStyle = new Color(c).hex;
		this.ctx.beginPath();
		this.ctx.arc(x, y, r, rad(start), rad(end));
		this.ctx.stroke();
	}

	textWidth(text: string, fontSize?: number | string) {
		if (fontSize) {
			this.ctx.font = typeof fontSize == "number" ? `${fontSize}pt Anonymous Pro, monospace` : fontSize;
		} else {
			this.ctx.font = "12pt Anonymous Pro, monospace";
